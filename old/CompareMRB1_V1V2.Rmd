...
========================================================
<!---
use these command instead of the knit icon to have the data and work loaded into the R workspace
  library(knitr)
  knit('CompareMRB1_V1V2.Rmd')
-->
Data Definitions 
-------------------------
WBID_V1V2 is a data frame with 28136 obs. and 4 variables

**WBID_V1V2 Data Definitions:**
...
Introduction:
-------------------------
We have two versions of the lakes of MRB1 (Major River Basin 1: HUC regions 01 and 02) from NHDplus.  These are MRB1_V1 and MRB1_V2. Need to compare the two version to see if the WB_IDs (unique identifiers), lake locations, and areas match.  The output will be a table that crosswalks the WB_IDs from V1 to V2.
...
#Start crosswalk list WBID in V1 and V2
  a<-comp[comp$flag==0,]
  WBID_V1V2<-data.frame(cbind(WBID_V1=a$WBID,WBID_V2=a$WBID))
    WBID_V1V2$flag<-0  #no flag for row.  
    WBID_V1V2$comment<-'Same Lake; WBID_V1==WBID_V2 and lake areas match' #N=27819
  ...
  #get counts for WBID's-check for multiple listings\
    ...
    #add matchV1V2$flag==0 WBIDs to WBID_V1V2
      ...
      WBID_V1V2<-rbind(WBID_V1V2,a)
          nrow(WBID_V1V2)  #27819+282=28101
...
#Create a spatialpolygons data.frame with the lakes from chkV1V2; reproject to WGS84
      table(V1$WB_ID%in%WBID_V1V2$WBID_V1) #21 lakes to check in V1
  ...
  #Get the lakes to check from V2
      table(V2$COMID%in%WBID_V1V2$WBID_V2) #29 lakes to check in V2
...
* `r Rows<-c(19:20);Zoom<-11`For **chkV1V2[c(`r Rows`),]** 
  ...
  * WBID_v2=c(`r unique(chkV1V2[Rows,'WBID_V2'])`)
* Plotting the lakes (see below) indicates they are contiguous and could be joined or separate.
* Add WBIDs to WBID_V1V2 with flag=2 indicating multiple lakes in V1 map to a single lake in V2
...
* Add WBIDs to WBID_V1V2 with flag=3 and the comment: Lakes collocated but areas unequal
...
```{r 27b, include=TRUE, echo=FALSE, cache=FALSE}
      par(mfrow=c(1,1))
...
```{r a1, include=TRUE, echo=FALSE, cache=FALSE}
          plotVicinity(1,4782861,.5)
```

```{r a2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(1,4782861,Zoom=12) 
```

```{r a3, include=TRUE, echo=TRUE, cache=FALSE}
            Row<-nrow(WBID_V1V2)+1 #row to add
              WBID_V1V2[Row,'WBID_V1']<-4782861
              WBID_V1V2[Row,'flag']<-4
              WBID_V1V2[Row,'comment']<-'In V1 but not V2.'
```

```{r c1, include=TRUE, echo=FALSE, cache=FALSE}
          plotVicinity(1,22287527,.5)
```

```{r c2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(1,22287527,Zoom=14) 
```

```{r c3, include=TRUE, echo=TRUE, cache=FALSE}
            Row<-nrow(WBID_V1V2)+1 #row to add
              WBID_V1V2[Row,'WBID_V1']<-22287527
              WBID_V1V2[Row,'flag']<-4
              WBID_V1V2[Row,'comment']<-'In V1 but not V2.'
```

```{r d1, include=TRUE, echo=FALSE, cache=FALSE}
          plotVicinity(1,22287665,1)
```

```{r d2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(1,22287665,Zoom=16) 
```

```{r d3, include=TRUE, echo=TRUE, cache=FALSE}
            Row<-nrow(WBID_V1V2)+1 #row to add
              WBID_V1V2[Row,'WBID_V1']<-22287665
              WBID_V1V2[Row,'flag']<-4
              WBID_V1V2[Row,'comment']<-'In V1 but not V2.'
```

```{r e1, include=TRUE, echo=FALSE, cache=FALSE}
          plotVicinity(1,15516920,1)
```

```{r e2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(1,15516920,Zoom=16) 
```

```{r e3, include=TRUE, echo=TRUE, cache=FALSE}
            Row<-nrow(WBID_V1V2)+1 #row to add  
              WBID_V1V2[Row,'WBID_V2']<-15516920
              WBID_V1V2[Row,'flag']<-5
              WBID_V1V2[Row,'comment']<-'In V2 but not V1.'
```

```{r f1, include=TRUE, echo=FALSE, cache=FALSE}
          plotVicinity(2,15516922,1)
```

```{r f2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(2,15516922,Zoom=16) 
```

```{r f3, include=TRUE, echo=TRUE, cache=FALSE}
            Row<-nrow(WBID_V1V2)+1 #row to add  
              WBID_V1V2[Row,'WBID_V2']<-15516922
              WBID_V1V2[Row,'flag']<-5
              WBID_V1V2[Row,'comment']<-'In V2 but not V1.'
```

```{r g1, include=TRUE, echo=FALSE, cache=FALSE}
          plotVicinity(2,60444415,1)
```

```{r g2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(2,60444415,Zoom=15) 
```

```{r g3, include=TRUE, echo=TRUE, cache=FALSE}
            Row<-nrow(WBID_V1V2)+1 #row to add  
              WBID_V1V2[Row,'WBID_V2']<-60444415
              WBID_V1V2[Row,'flag']<-5
              WBID_V1V2[Row,'comment']<-'In V2 but not V1.'
```
...
* The match between V1 and V2 WBID=22302965 WBID_V1V2 was flagged and commented
* A new entry in WBID_V1V2 showing the link between V1_WBID=10312598 and V2_WBID=22302965 was added.
* Here are the graphics for V1_WBID=10312598:

  
```{r b1, include=TRUE, echo=FALSE, cache=FALSE}
    wbid<-10312598;scale<-2.5;version<-1;
          plotVicinity(version,wbid,scale)
```

```{r b2, include=TRUE, echo=FALSE, cache=FALSE}
          plotGmap(version,wbid,Zoom=10) 
      #update original line in WBID_V1V2
        WBID_V1V2[WBID_V1V2$WBID_V1==22302965,'flag']<-2
        WBID_V1V2[WBID_V1V2$WBID_V1==22302965,'comment']<-'Two Lakes in V1 represented as a single lake in V2'
```

```{r b3, include=TRUE, echo=FALSE, cache=FALSE}
      #add new line to WBID_V1V2
        Row<-nrow(WBID_V1V2)+1 #row to add
          WBID_V1V2[Row,'WBID_V1']<-10312598
          WBID_V1V2[Row,'WBID_V2']<-22302965
          WBID_V1V2[Row,'flag']<-2
          WBID_V1V2[Row,'comment']<-'Two Lakes in V1 represented as a single lake in V2'
...
* Need to update WBID_V1V2 to reflect this adding a new flag code #5 to explain what happened.


```{r h2, include=TRUE, echo=FALSE, cache=FALSE}
  par(mfrow=c(1,2))
...
```{r h3, include=TRUE, echo=FALSE, cache=FALSE}
    #edit WBID_V1V2
      Comment<-'V2 Duplicate lake; V2_166421080=V2_19333669=V1_19333669'
        WBID_V1V2[which(WBID_V1V2$WBID_V1==19333669),]
        WBID_V1V2[which(WBID_V1V2$WBID_V1==19333669),'flag']<-6
        WBID_V1V2[which(WBID_V1V2$WBID_V1==19333669),'comment']<-Comment
    #add new line to WBID_V1V2
      Row<-nrow(WBID_V1V2)+1 #row to add
        WBID_V1V2[Row,'WBID_V1']<-19333669
        WBID_V1V2[Row,'WBID_V2']<-166421080
        WBID_V1V2[Row,'flag']<-6
        WBID_V1V2[Row,'comment']<-Comment

```

QAQC-Check to make sure all V1 and V2 WBIDs are included in WBID_V1V2
-------------------------
* All V1 and V2 WBIDs are in WBID_V1V2

```{r Q1, include=TRUE, echo=TRUE, cache=FALSE}
table(V1$WB_ID%in%WBID_V1V2$WBID_V1)
table(V2$COMID%in%WBID_V1V2$WBID_V2)
```

* Checking if all WBID_V1V2 WBIDs are in V1 and V2 shows that for each Version 3 are missing
* For each version 3 lakes are found in one version and not the next so the mis-matches are NAs.

```{r Q2, include=TRUE, echo=TRUE, cache=FALSE}
table(WBID_V1V2$WBID_V1%in%V1$WB_ID)
  WBID_V1V2[which(WBID_V1V2$WBID_V1%in%V1$WB_ID==FALSE),]
table(WBID_V1V2$WBID_V2%in%V2$COMID)
  WBID_V1V2[which(WBID_V1V2$WBID_V2%in%V2$COMID==FALSE),]
```

Save the data
-------------------------
* WBID_V1V2 save to './tables/WBID_V1V2.rda'

```{r Save, include=FALSE, echo=FALSE, cache=FALSE}
  #save the data
    save(WBID_V1V2,file='./tables/WBID_V1V2.rda')
```
